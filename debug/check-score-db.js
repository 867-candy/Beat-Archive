...
const dbPath = 'C:\\Users\\yuhi-dosei\\Downloads\\score.db';

console.log('Checking corrected score database...');
  ...
  if (hasScoreTable) {
    ...
    db.all(`PRAGMA table_info(score)`, (err, columns) => {
      if (err) {
        ...
        // いくつかのレコードがあるかチェック
        db.get('SELECT COUNT(*) as count FROM score', (err, row) => {
          if (err) {
            console.error('Error counting records:', err);
          } else {
            console.log(`\nNumber of records in score table: ${row.count}`);
          }
          db.close();
        });
      }
