...
function calculateNextDjLevelPoints(currentScore, maxScore) {
  const scoreRate = maxScore > 0 ? (currentScore / maxScore) * 100 : 0;
  ...
  for (const threshold of thresholds) {
    if (scoreRate < threshold.min) {
      const requiredScore = Math.ceil((threshold.min / 100) * maxScore);
      const pointsNeeded = requiredScore - currentScore;
      return {
        nextLevel: threshold.level,
        pointsNeeded: Math.max(0, pointsNeeded),
        requiredRate: threshold.min
      };
    }
  }
    ...
    for (const row of logs) {
      ...
      if (hasUpdate) {
        ...
        result.push({
          ...
          nextDjLevelPoints,
          totalNotes: currentBest.notes || 0,
          updates,
          playDate: row.date,
          // 難易度表情報
